...
const Auth1 = (props) => {
  ...
  const submit = async (e) => {
    ...
    try {
      if (isSignup) {
        ...
        try {
          await dispatch(signin(e, checked));
  ...
  const googleSuccess = async (res) => {
    ...
    try {
      const { data } = await api.google({ tokenId: token });
  ...
  const responseFacebook = async (response) => {
    const { data } = await api.facebook({ access_token: response.accessToken });
    console.log(response.accessToken, data);
    dispatch({ type: AUTH, data });
    history.push("/dashboard");
  };
            ...
            <Grid container spacing={0}>
              <Grid item xs={12} sm={6} md={6}>
                <Paper className={classes.paper}>
                  <ButtonGroup orientation="vertical" className={classes.test}>
                    <FacebookLogin
              ...
              <Grid item xs={12} sm={6} md={6}>
                        ...
                        <Form>
                          ...
                          >
                            {isSignup ? (
                              ...
                              <Typography className={classes.subtitle}>
                                Sign in with Your{" "}
                                <span className={classes.spanText}>Admin</span>{" "}
                                Account
                              </Typography>
                            )}
                          </div>
                ...
                >
                  {isSignup && (
                    <>
                      <Typography className={classes.privacy1}>
                        By Creating your GottaMenu Account, You agree to the
                      </Typography>
                      <Typography className={classes.privacy2}>
                        {" "}
                        <span className={classes.spanText}> Terms of use </span>
                        &nbsp;and&nbsp;
                        <span className={classes.spanText}>
                          {" "}
                          Privacy Policy
